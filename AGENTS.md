...
- `uv run --python 3.11 --exclude-newer P7D --with numpy --with pandas --with psutil --with scikit-learn --with xgboost --with lightgbm --with matplotlib --with threadpoolctl --with py-spy python <script>.py`
...
- For full benchmark + profiling smoke execution:
  - `uv run --python 3.11 --exclude-newer P7D --with numpy --with pandas --with psutil --with scikit-learn --with xgboost --with lightgbm --with matplotlib --with threadpoolctl --with py-spy python hgbdt_performance_analysis/run_ci_benchmarks_profiles.py --machine-tag <tag> --artifacts-root hgbdt_performance_analysis/artifacts`
