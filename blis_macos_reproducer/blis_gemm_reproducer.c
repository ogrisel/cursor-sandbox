/*
 * Minimal C reproducer: cblas_dgemm vs scalar long-double reference.
 * Intended to link against upstream BLIS (see build_and_run_c_blis_reproducer.sh).
 */
...
int main(void)
{
    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            ...
            long double diff =
                fabsl((long double)c[(size_t)i * (size_t)n + (size_t)j] - acc);
            if (diff > max_abs_err) {
                max_abs_err = diff;
            }
